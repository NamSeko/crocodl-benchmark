...
def writer_roma_fn(inp, match_path):
    ...
    with h5py.File(str(match_path), 'a', libver='latest') as fd:
        if pair in fd:
            del fd[pair]
        grp = fd.create_group(pair)
        keypoints0 = pred['keypoints0'].cpu().short().numpy()
        grp.create_dataset('keypoints0', data=keypoints0)
        keypoints1 = pred['keypoints1'].cpu().short().numpy()
        grp.create_dataset('keypoints1', data=keypoints1)
        if 'mconf' in pred:
            mconf = pred['mconf'].cpu().half().numpy()
...
def match_from_paths(conf: Dict,
    ...
    if conf['model']['name'] == 'roma':
        ...
        writer_queue = WorkQueue(partial(writer_roma_fn, match_path=match_path), 5)
        for idx, data in enumerate(tqdm(loader, smoothing=.1)):
            ...
            img0 = Image.fromarray((data0['image'][0].cpu().numpy() * 255).astype(np.uint8))
            img1 = Image.fromarray((data1['image'][0].cpu().numpy() * 255).astype(np.uint8))
            img0_size = data0['image_size']
            img1_size = data1['image_size']
            img0_size = tuple(img0_size[0].tolist())
            img1_size = tuple(img1_size[0].tolist())

            data0 = {k: v for k, v in data0.items() if k not in ["image", "image_size"]}
            data1 = {k: v for k, v in data1.items() if k not in ["image", "image_size"]}
            pred = model([data0, img0, img0_size, data1, img1, img1_size])
